...
class BuildGrau(QWidget, Ui_buildGraular):
    ...
    # 第一个是视图信息，另一个是层次信息
    subWindoSignel = pyqtSignal(str, list, list, list, bool, list, str)
    ...
    def okConfig(self):
        ...
        data_info, view_index, ci_b_list, res_list, ci_list, multi_ans = build_Graular(view_div, ci_div, self.df2)
        # 将返回的结果构建成为字符串返回给父窗口
        # 传递信息
        self.subWindoSignel.emit(data_info, view_index, ci_list, res_list, False, ci_b_list, multi_ans)
        self.close()
        pass
    ...
    def okCi(self):
        ...
        data_info, view_index, ci_list, res_list, view_atr, multi_ans = build_Graular_Value(list_view, self.df, self.df2)
        self.subWindoSignel.emit(data_info, view_index, ci_list, res_list, True, view_atr, multi_ans)
        self.close()
        pass
