...
def build_Graular(view, ci, df2):
    ...
    for i, data in enumerate(ci_b_list):  # 视图信息作为下标
        ...
        for j, d in enumerate(data):  # 层次信息作为上标
            if j != len(data) - 1:
                str_data = str_data + "π<sub>{}</sub><sup>{}</sup>:{},\t".format(i + 1, j + 1, getgroup(df2, d))
                pass
            else:
                str_data = str_data + "π<sub>{}</sub><sup>{}</sup>:{}".format(i + 1, j + 1, getgroup(df2, d))
                pass
        ...
        pass
    str_data += "<hr/>"
    str_data += "<p>多视角多层次粒结构:</p>"
    multi_ans = "<p>多视角多层次粒结构:</p>"
    ...
    for res in res_list:
        ...
        for r in res:
            ...
            for i in range(len(r)):
                ...
                multi_ans = multi_ans +  "π<sub>{}</sub><sup>{}</sup>".format(i+1, r[i]+1)
                if i == len(r) - 1:
                    str_data = str_data + ")"
                    multi_ans = multi_ans + ")"
                    pass
        ...
        multi_ans += '</p>'
        pass
    return str_data, result, ci_b_list, res_list, ci_list, multi_ans
    pass
...
def build_Graular_Value(list_view, df, df2):
    ...
    for i, data in enumerate(ci_e_list):  # 视图信息作为下标
        ...
        for j, d in enumerate(data):  # 层次信息作为上标
            if j != len(data)-1:
                str_ans = str_ans + "π<sub>{}</sub><sup>{}</sup>:{},\t".format(i + 1, j + 1,
    ...
    return str_ans, [], list_view, res_list, ci_e_list, multi_ans
    pass
...
def showFinal(level_red, best, df, tag):
    '''
...
def show_acc(level_red, best, df, methods, tag):
    #  求出df2
    fn = len(df.columns)  # 染色体长度
    obn = len(df)  # 数据集样本个数
    # print(fn, obn)
...
def acc(attr_data, df2, methods):
    ...
    # accuracyavg = accuracy_score(lable, tree_predict)
    print(accuracyavg)
    return accuracyavg
    pass
